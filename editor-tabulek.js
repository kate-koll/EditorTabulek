...
function createDefaultTable(rows, columns) {
  ...
  const firstActiveCell = document.getElementsByTagName("input");
  firstActiveCell[0].focus();
}
...
function createDefaultButtons() {
  createAndInsertButton("Přidat řádek dolů", document.body).onclick =
    addRowBelow;
  createAndInsertButton("Přidat řádek nahoru", document.body).onclick =
    addRowAbove;
  createAndInsertButton("Přidat sloupec vlevo", document.body).onclick =
    addColumnLeft;
  createAndInsertButton("Přidat sloupec vpravo", document.body).onclick =
    addColumnRight;
  createAndInsertButton("Odstranit řádek", document.body).onclick = deleteRow;
  createAndInsertButton("Odstranit sloupec", document.body).onclick =
    deleteColumn;
}
...
function addColumnLeft() {
  ...
  for (let i = 0; i < table.childNodes.length; i++) {
    table.childNodes[i].insertBefore(
      createCell(),
      table.childNodes[i].childNodes[activeIndex]
    );
  }
}
...
function addColumnRight() {
  ...
  for (let i = 0; i < table.childNodes.length; i++) {
    if (
      table.childNodes[i].childNodes[activeIndex] ==
      table.childNodes[i].lastElementChild
    ) {
      table.childNodes[i].appendChild(createCell());
    } else {
      table.childNodes[i].insertBefore(
        createCell(),
        table.childNodes[i].childNodes[activeIndex + 1]
      );
    }
  }
}
...
function deleteColumn() {
  let activeIndex = findActiveCellColumnIndex();
  for (let i = 0; i < table.childNodes.length; i++) {
    table.childNodes[i].removeChild(
      table.childNodes[i].childNodes[activeIndex]
    );
  }
}
